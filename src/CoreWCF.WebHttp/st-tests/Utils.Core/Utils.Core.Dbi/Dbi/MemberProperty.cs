...
namespace ST.Utils
{
  public partial class Dbi
  {
    ...
    private class MemberProperty
    {
      ...
      private static readonly List<BindBaseAttribute> _dynamicBindEmpty = new List<BindBaseAttribute>();
      internal static readonly ConcurrentDictionary<ulong, List<BindBaseAttribute>> DynamicBinds = new ConcurrentDictionary<ulong, List<BindBaseAttribute>>();
      #endregion
      ...
      private static List<MemberEntry> GetMembers( Type type, string name, ConcurrentDictionary<Type, Dictionary<string, List<MemberEntry>>> cache )
      {
        var d = cache.GetOrAdd( type, t =>
        {
          ...
          foreach( var pi in t.GetProperties( BindingFlags.Instance | BindingFlags.Public ) )
          {
            ...
            if( !(dynamicBind.Exists( b => b.GetType() == typeof( BindNoneAttribute ))) &&
                ((cache == _in && !dynamicBind.Exists( b => b is BindInNoneAttribute)) || (cache == _out && !dynamicBind.Exists( b => b is BindOutNoneAttribute))) &&
                !pi.IsDefined<BindNoneAttribute>( false ) &&
                !(cache == _in ? pi.IsDefined<BindInNoneAttribute>( false ) : pi.IsDefined<BindOutNoneAttribute>( false )) )
            {
              var property = new MemberProperty( pi );

              var bindAll = (dynamicBind.FirstOrDefault( b => b is BindAttribute ) ?? pi.GetAttribute<BindAttribute>()) as BindAttribute;

              var memberNames = bindAll != null ? bindAll.Names : new List<string> { pi.Name.ToUpper() };

              if( cache == _in )
                memberNames = memberNames.Select( n => "@" + n ).ToList();

              var binds = (cache == _in ? dynamicBind.Where( b => b is BindInExAttribute ) : dynamicBind.Where( b => b is BindOutExAttribute )).Cast<BindExAttribute>().ToArray();

              if( binds.Length == 0 )
                binds = pi.GetCustomAttributes( cache == _in ? typeof( BindInExAttribute ) : typeof( BindOutExAttribute ), true ) as BindExAttribute[];
              ...
              var bind = (cache == _in ? dynamicBind.FirstOrDefault( b => b is BindInAttribute ) : dynamicBind.FirstOrDefault( b => b is BindOutAttribute )) as BindNonEmptyAttribute;

              if( bind == null )
                bind = Attribute.GetCustomAttribute( pi, cache == _in ? typeof( BindInAttribute ) : typeof( BindOutAttribute ), true ) as BindNonEmptyAttribute;

              memberNames = (bind == null ? memberNames : bind.Names);
