...
namespace ST.Utils
{
  public partial class Dbi_PG
  {
    ...
    internal sealed class PgsqlExceptionHandlerAttribute : OnExceptionAspect
    {
      #region CompileTimeValidate
      public override bool CompileTimeValidate(MethodBase method)
      {
        if (!typeof(IConnectionHolder_PG).IsAssignableFrom(method.DeclaringType))
      ...
      {
        var exc = (NpgsqlException)args.Exception;

        var ch = (IConnectionHolder_PG)args.Instance;

        var server = ch.Connection.Host?.GetEmptyOrTrimmed() + ":" + ch.Connection.Port;
        var database = ch.Connection.Database.GetEmptyOrTrimmed();

        if (exc is PostgresException postgresExc)
        {
          if (PgDatabaseConnectionException.IsConnectionError(postgresExc.SqlState))
            throw new PgDatabaseConnectionException(exc, server, database) as Exception;
          ...
          switch (postgresExc.SqlState)
          {
            ...
            //case "23505": // Такой же код как у предыдущей ошибки 2601 (в PostgreSQL это общая ошибка).
            //  throw new UniqueConstraintViolationException_2627( exc, server, database ) as Exception;
            //case "":       // Нет аналога для 1919 в PostgreSQL
            ...
            //case "42804": // Такой же код как у предыдущей ошибки 206 (в PostgreSQL это общая ошибка).
