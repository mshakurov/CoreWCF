...
//{
//  /// <summary>
//  /// Класс для работы с базой данных PostgreSQL через интерфейс хранимых процедур.
//  /// </summary>
//  [PgsqlExceptionHandler]
//  public sealed partial class Dbi_PG : IDbi, IConnectionHolder_PG
//  {
//    #region .Constants
//    private const string CONNECTION_TIMEOUT = "Timeout";

//    private const string RESERVE_DBI_MESSAGE = "\"{0}\" - Using Reserve Dbi for stored procedure \"{1}\" because of error: {2}";
//    #endregion

//    #region .Fields
//    private readonly NpgsqlConnectionStringBuilder _connection = new NpgsqlConnectionStringBuilder();
...
//        //_connection.Host = _connection.Host.ToUpper();
//        //_connection.Database = _connection.Database.ToUpper();
//      }
//    }
...
//    /// <summary>
//    /// Резервный DBi для работы с резервной БД.
//    /// </summary>
//    public IDbi ReserveDbi
//    {
//      get;
//      set;
//    }
...
//    private static readonly ConcurrentDictionary<string, Func<Dbi_PG, CommandBehavior?, object[], IDbiDataProvider>> _replaceActions = new ConcurrentDictionary<string, Func<Dbi_PG, CommandBehavior?, object[], IDbiDataProvider>>();
...
//          try
//          {
//            var comm = new NpgsqlCommand( "select 1", conn );
...
//        catch( Exception e )
//        {
//          var server = conn.Host.GetEmptyOrTrimmed() + ":" + conn.Port;
//          var database = conn.Database.GetEmptyOrTrimmed();

//          exc = new PgDatabaseConnectionException( e, server, database );
//        }
...
//    [DebuggerStepThrough]
//    private static Func<Dbi_PG, CommandBehavior?, object[], IDbiDataProvider> GetReplaceAction( string name )
//    {
...
//    {
//      using( var sp = new StoredProcedure( this, true, name, args ) )
//        return new ExecuteAsyncResult( sp.PgsqlCommand, sp.PgsqlCommand.ExecuteNonQueryAsync() );
//    }
//    #endregion
...
//    [DebuggerStepThrough]
//    private static object GenerateObject( Dbi_PG dbi, XmlReader node, string elementName, GeneratingData data )
//    {
//      object obj = null;
...
//                  me.Property.Set( obj, GenerateObject( dbi, node.ReadSubtree(), node.Name, new GeneratingData( dbi, me.Property.MemberType, data.Name, true ) ) );
//                else
//                {
...
//                  var innerData = new GeneratingData( dbi, me.Property.ElementType, data.Name, true );
...
//                      listType.Add( items, GenerateObject( dbi, subnodes.ReadSubtree(), elementTypeName, innerData ) );
//                  };
...
//    /// <returns>Скалярное значение.</returns>
//    [DebuggerStepThrough]
//    public T GetScalarInternal<T>( string name, params object[] args )
//    {
//      var replaceAction = GetReplaceAction( name );
...
//    {
//      var res = GetScalarInternal<T>( name, args );

//      var replaceAction = GetReplaceAction( name );

//      if( this.ReserveDbi != null && replaceAction == null )
//      {
//        var oldRes = this.ReserveDbi.GetScalar<T>( name, args );

//        CheckIsEqual( res, oldRes, name, "Dbi_PG.GetScalar" );
//      }
...
//    [DebuggerStepThrough]
//    public List<T> GetScalarListInternal<T>( string name, IEnumerable objects )
//    {
...
//    {
//      var res = GetScalarListInternal<T>( name, objects );
      
//      var replaceAction = GetReplaceAction( name );

//      if( this.ReserveDbi != null && replaceAction == null )
//      {
//        var oldRes = this.ReserveDbi.GetScalarList<T>( name, objects );

//        CheckIsEqual( res, oldRes, name, "Dbi_PG.GetScalarList" );
//      }
...
//    [DebuggerStepThrough]
//    public DataTable GetTableInternal( string name, params object[] args )
//    {
...
//    {
//      var res = GetTableInternal( name, args );

//      var replaceAction = GetReplaceAction( name );

//      if( this.ReserveDbi != null && replaceAction == null )
//      {
//        var oldRes = this.ReserveDbi.GetTable( name, args );

//        CheckIsEqual( res, oldRes, name, "Dbi_PG.GetTable" );
//      }
...
//        while( sp.Reader.Read() )
//          //sb.Append( sp.Reader.ReadOuterXml() );
//          sb.Append( sp.Reader.GetTextReader( 0 ).ReadToEnd() );
...
//    [DebuggerStepThrough]
//    public static void SetBind<T>( Expression<Func<T, object>> e, params Dbi.BindBaseAttribute[] binding )
//    {
...
//    [DebuggerStepThrough]
//    internal static void RegisterAction<T>( DbiType dbiType, string spName, Func<T, CommandBehavior?, object[], IDbiDataProvider> replaceAction ) where T : IDbi
//    {
//      if( dbiType == DbiType )
//      {
//        if( typeof( T ) != typeof( Dbi_PG ) )
//          throw new ArgumentException( String.Format( "'{0}' is incorrect Dbi type for selected DbiType enum value '{1}'. Should be '{2}'.", typeof( T ), dbiType.ToString(), typeof( Dbi_PG ) ) );

//        if( !String.IsNullOrEmpty( spName ) && replaceAction != null )
//        {
//          Func<Dbi_PG, CommandBehavior?, object[], IDbiDataProvider> replaceActionWrapper = ( Dbi_PG dbi, System.Data.CommandBehavior? behavior, object[] args ) =>
//          {
...
//              return ( replaceAction as Func<Dbi_PG, CommandBehavior?, object[], IDbiDataProvider> )( dbi, behavior, args );
//            }
//            finally
//            {
...
//    #region IConnectionHolder_PG
//    NpgsqlConnectionStringBuilder IConnectionHolder_PG.Connection
//    {
//      [DebuggerStepThrough]
...
//    /// </summary>
//    [PgsqlExceptionHandler]
//    public sealed class RSResult : IRSResult, IConnectionHolder_PG
//    {
//      #region .Fields
...
//      private static Dbi.RSResult.PartialObject<T> GeneratePartialObject<T>( DbDataReader reader, GeneratingData data, List<ReaderData> items, List<int> notBounded, bool nullAsDefault = false )
//        where T : class, new()
//      {
//        var obj = new Dbi.RSResult.PartialObject<T> { Target = GenerateObject( reader, data, items, nullAsDefault ) as T };
...
//      [DebuggerStepThrough]
//      public List<T> ListInternal<T>( string name, params object[] args )
//        where T : class, new()
//      {
//        try
//        {
...
//          var data = new GeneratingData( _dbi, typeof( T ), name, false );
...
//        catch( Exception ex )
//        {
//          if( this._dbi.MixedDbMode && this._dbi.ReserveDbi != null )
//          {
//            Console.WriteLine( RESERVE_DBI_MESSAGE, "Dbi_PG.RS.List", name, ex.Message );  //WriteToLog()

//            return this._dbi.ReserveDbi.RS.List<T>( name, args );
//          }
//          else
//            throw;
//        }
...
//        where T : class, new()
//      {
//        var res = ListInternal<T>( name, args );

//        var replaceAction = GetReplaceAction( name );

//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.RS.List<T>( name, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.RS.List" );
//        }
...
//      /// <returns>Список воссозданных объектов типа T.</returns>
//      [DebuggerStepThrough]
//      public List<T> ListDefInternal<T>( string name, params object[] args )
//        where T : class, new()
//      {
//        var list = new List<T>();
...
//          while( sp.Reader.Read() )
//            list.Add( GenerateObject( sp.Reader, data, items, true ) as T );
//        }
...
//        where T : class, new()
//      {
//        var res = ListDefInternal<T>( name, args );

//        var replaceAction = GetReplaceAction( name );

//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.RS.ListDef<T>( name, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.RS.ListDef" );
//        }
...
//          while( sp.Reader.Read() )
//          {
//            var genDataBased = dictGenDataDerived.GetOrAdd( typeof( TBase ), type => new GeneratingData( _dbi, type, name, false ) );
...
//              genDataDerived = dictGenDataDerived.GetOrAdd( typeDerived, type => new GeneratingData( _dbi, type, name, false ) );
//            }
...
//      [DebuggerStepThrough]
//      public List<Dbi.RSResult.PartialObject<T>> ListPartialInternal<T>( string name, params object[] args )
//        where T : class, new()
//      {
//        try
//        {
//          var list = new List<Dbi.RSResult.PartialObject<T>>();
...
//          if( this._dbi.MixedDbMode && this._dbi.ReserveDbi != null )
//          {
//            Console.WriteLine( RESERVE_DBI_MESSAGE, "Dbi_PG.RS.ListPartial", name, ex.Message );  //WriteToLog()

//            return this._dbi.ReserveDbi.RS.ListPartial<T>( name, args );
//          }
//          else
//            throw;
//        }
...
//      public List<Dbi.RSResult.PartialObject<T>> ListPartial<T>( string name, params object[] args )
//        where T : class, new()
//      {
//        var res = ListPartialInternal<T>( name, args );

//        var replaceAction = GetReplaceAction( name );

//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.RS.ListPartial<T>( name, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.RS.ListPartial" );
//        }
...
//      /// <returns>Список частично воссозданных объектов типа T.</returns>
//      [DebuggerStepThrough]
//      public List<Dbi.RSResult.PartialObject<T>> ListPartialDef<T>( string name, params object[] args )
//        where T : class, new()
//      {
//        var list = new List<Dbi.RSResult.PartialObject<T>>();
...
//      [DebuggerStepThrough]
//      public List<T> ListScalarInternal<T>( string name, params object[] args )
//      {
//        try
//        {
//          var list = new List<T>();
...
//          if( this._dbi.MixedDbMode && this._dbi.ReserveDbi != null )
//          {
//            Console.WriteLine( RESERVE_DBI_MESSAGE, "Dbi_PG.RS.ListScalar", name, ex.Message );  //WriteToLog()

//            return this._dbi.ReserveDbi.RS.ListScalar<T>( name, args );
//          }
//          else
//            throw;
//        }
...
//      {
//        var res = ListScalarInternal<T>( name, args );

//        var replaceAction = GetReplaceAction( name );

//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.RS.ListScalar<T>( name, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.RS.ListScalar" );
//        }
...
//      [DebuggerStepThrough]
//      public T SingleInternal<T>( string name, params object[] args )
//        where T : class, new()
//      {
//        try
//        {
...
//          if( this._dbi.MixedDbMode && this._dbi.ReserveDbi != null )
//          {
//            Console.WriteLine( RESERVE_DBI_MESSAGE, "Dbi_PG.RS.Single", name, ex.Message );  //WriteToLog()

//            return this._dbi.ReserveDbi.RS.Single<T>( name, args );
//          }
//          else
//            throw;
//        }
...
//        where T : class, new()
//      {
//        var res = SingleInternal<T>( name, args );
        
//        var replaceAction = GetReplaceAction( name );

//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.RS.Single<T>( name, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.RS.Single" );
//        }
...
//      [DebuggerStepThrough]
//      public TBase SingleOfBaseDefInternal<TBase>( string name, Func<TBase, Type> derivedTypeGetter, params object[] args )
//        where TBase : class, new()
//      {
//        //var data = new GeneratingData( _dbi, typeof( T ), name, false );
...
//          if( sp.Reader.Read() )
//          {
//            var genDataBased = new GeneratingData( _dbi, typeof( TBase ), name, false );

//            TBase baseObject = GenerateObject( sp.Reader, genDataBased, ReaderData.Get( sp.Reader, genDataBased.Members ), true ) as TBase;

//            var typeDerived = derivedTypeGetter( baseObject );

//            var genDataDerived = new GeneratingData( _dbi, typeDerived, name, false );
...
//        where TBase : class, new()
//      {
//        var res = SingleOfBaseDefInternal<TBase>( name, derivedTypeGetter, args );

//        var replaceAction = GetReplaceAction( name );

//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.RS.SingleOfBaseDef<TBase>( name, derivedTypeGetter, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.RS.SingleOfBaseDef" );
//        }
...
//      [DebuggerStepThrough]
//      public Dbi.RSResult.PartialObject<T> SinglePartialInternal<T>( string name, params object[] args )
//        where T : class, new()
//      {
//        try
//        {
...
//        catch( Exception ex )
//        {
//          if( this._dbi.MixedDbMode )
//          {
//            Console.WriteLine( RESERVE_DBI_MESSAGE, "Dbi_PG.RS.SinglePartial", name, ex.Message );  //WriteToLog()

//            return this._dbi.ReserveDbi.RS.SinglePartial<T>( name, args );
//          }
//          else
//            throw;
//        }
...
//      public Dbi.RSResult.PartialObject<T> SinglePartial<T>( string name, params object[] args )
//        where T : class, new()
//      {
//        var res = SinglePartialInternal<T>( name, args );

//        var replaceAction = GetReplaceAction( name );

//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.RS.SinglePartial<T>( name, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.RS.SinglePartial" );
//        }
...
//      {
//        [DebuggerStepThrough]
//        get { return ( _dbi as IConnectionHolder_PG ).Connection; }
//      }
//      #endregion
...
//    /// </summary>
//    [PgsqlExceptionHandler]
//    public sealed class XmlResult : IXmlResult, IConnectionHolder_PG
//    {
//      #region .Fields
//      private readonly Dbi_PG _dbi;
//      #endregion
...
//          var data = new GeneratingData( _dbi, typeof( T ), name, true );
...
//            if( xmlSp.Reader.Read() && xmlSp.Reader.NodeType == XmlNodeType.Element )   // root "ROOT_XML_NODE"
//            {
//              while( xmlSp.Reader.Read() && xmlSp.Reader.NodeType != XmlNodeType.EndElement )
//                list.Add( ( T ) GenerateObject( _dbi, xmlSp.Reader.ReadSubtree(), typeof( T ).Name, data ) );
//            }
//          }
...
//          if( this._dbi.MixedDbMode && this._dbi.ReserveDbi != null )
//          {
//            Console.WriteLine( RESERVE_DBI_MESSAGE, "Dbi_PG.Xml.List", name, ex.Message );  //WriteToLog()

//            return this._dbi.ReserveDbi.Xml.List<T>( name, args );
//          }
//          else
//            throw;
//        }
...
//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.Xml.List<T>( name, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.Xml.List" );
//        }
...
//            if( xmlSp.Reader.Read() && xmlSp.Reader.NodeType == XmlNodeType.Element )     // root "ROOT_XML_NODE"
//              return ( T ) GenerateObject( _dbi, xmlSp.Reader, typeof( T ).Name, new GeneratingData( _dbi, typeof( T ), name, true ) );
...
//          if( this._dbi.MixedDbMode && this._dbi.ReserveDbi != null )
//          {
//            Console.WriteLine( RESERVE_DBI_MESSAGE, "Dbi_PG.Xml.Single", name, ex.Message );  //WriteToLog()

//            return this._dbi.ReserveDbi.Xml.Single<T>( name, args );
//          }
//          else
//            throw;
//        }
...
//        if( this._dbi.ReserveDbi != null && replaceAction == null )
//        {
//          var oldRes = this._dbi.ReserveDbi.Xml.Single<T>( name, args );

//          CheckIsEqual( res, oldRes, name, "Dbi_PG.Xml.Single" );
//        }
...
//    /////
//    // NOTE: For tests only!
//    public static bool CheckIsEqual( object obj1, object obj2, string spName, string functionName )
//    {
//      if( ReferenceEquals( obj1, obj2 ) )
//        return true;
//      if( ( obj1 == null ) || ( obj2 == null ) )
//      {
//        Console.WriteLine( "NotEqual result - one of objects is NULL! Function '{0}', stored procedure '{1}'", functionName, spName );

//        return false;
//      }
//      if( obj1.GetType() != obj2.GetType() )
//      {
//        Console.WriteLine( "NotEqual result - different TYPES! Function '{0}', stored procedure '{1}'", functionName, spName );

//        return false;
//      }

//      if( obj1 is IList && obj2 is IList && ( obj1 as IList ).Count != ( obj2 as IList ).Count )
//      {
//        Console.WriteLine( "NotEqual result - different COUNT! Function '{0}', stored procedure '{1}', obj1.Count={2}, obj1.Count={3}", functionName, spName, ( obj1 as IList ).Count, ( obj2 as IList ).Count );

//        return false;
//      }

//      //var objJson1 = JsonConvert.SerializeObject( obj1 );
//      //var objJson2 = JsonConvert.SerializeObject( obj2 );

//      //var isEqual = objJson1 == objJson2;

//      //if( !isEqual )
//      //  Console.WriteLine( "NotEqual DATA result! Function '{0}', stored procedure '{1}'", functionName, spName );
...
//      #region .Ctor
//      [DebuggerStepThrough]
//      public GeneratingData( Dbi_PG dbi, Type type, string name, bool complexTypeMembers )
//      {
//        Name = Utils.GetNormalizedName( name, dbi.OwnerModuleName );
...
//          Members = MemberProperty.GetOutMembers( type, Name ).Where( me => !me.Property.IsOutComplexType ).ToList();
//          ComplexMembers = complexTypeMembers ? MemberProperty.GetOutMembers( type, Name ).Where( me => me.Property.IsOutComplexType ).ToList() : null;
...
//        {
//          var name = reader.GetName( i ).ToUpper()/*ToLower? .ToUpper()*/;
...
//      internal ExecuteAsyncResult( NpgsqlCommand command, IAsyncResult asyncResult )
//      {
//        Command = command;
...
//  internal interface IConnectionHolder_PG
//  {
//    NpgsqlConnectionStringBuilder Connection { get; }
//  }
