...
namespace ST.Utils
{
  public static class PropertyComparer
  {
    ...
    public static bool EqualsProperties<T>( this T one, T second )
      where T : class
    {
      if ( one == null 
        || second == null  )
        return false;
      return GetComparer( typeof( T ) )( one, second );
    }
    ...
    public static Func<object, object, bool> GetComparer( Type type )
    {
      ...
      {
        var props = type.GetProperties().Where( p => p.CanRead && p.GetIndexParameters().Length == 0  ).ToList();
